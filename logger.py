...
def setup_logging(log_file=None, level='INFO', enable_file=True):
    """
    配置日志系统

    参数:
        log_file:    日志文件路径；为 None 时自动生成带时间戳的文件名
        level:       日志级别（默认 INFO）
        enable_file: 是否启用文件日志（默认 True）；
                     传入 False 可关闭文件输出（仅控制台）
    """
    if enable_file:
        # log_file=None 时 add_file_handler 内部自动生成路径
