...
- 📁 **统一输出目录**：所有生成文件（cleaned、removed、report）自动写入输入文件同级的 `Output/` 子目录，不污染原始目录
- ⌨️ **默认 Y 交互**：所有命令行提示均可直接按 Enter 接受默认值（Y），无需手动键入

### 🔧 技术改进

- ✅ **日志文件输出修复**：`setup_logging` 新增 `enable_file` 参数，修复原先文件日志未实际写入的问题，运行后自动在 `logs/` 目录生成带时间戳日志
- ✅ **相似术语去重优化**：O(n²) 线性扫描改为 O(1) set 查找，大规模数据集性能提升明显
- ✅ **代码去重**：提取 `_ask_config` 辅助函数，消除批量/单文件两处重复的交互配置代码
- ✅ **辅助函数重构**：相似术语处理中 term1/term2 重复块提取为 `_rows_for_term` / `_add_similar_entry` 嵌套函数
- ✅ **冗余代码清理**：移除 `traceback.print_exc()` 重复打印（`logger.error(exc_info=True)` 已记录完整堆栈）

### 📦 新增/变更

- `Output/` 子目录（运行后自动创建）
- `logs/` 子目录（运行后自动创建，带时间戳日志文件）
