...
def clean_terminology_table(input_file, output_clean=None, output_removed=None, output_uncertain=None, config=None):
    ...
    if config.similarity_check:
        ...
        if all_similar:
            ...
            # 统计聚类信息
            total_terms_in_clusters = len(
                {p['术语1'] for p in all_similar} | {p['术语2'] for p in all_similar}
            )
            print(f"  聚类结果：{total_terms_in_clusters}个相似术语归入{len(clusters)}个相似组")
            
            # 处理每对相似术语，使用聚类的组ID
            similar_rows_seen = set()

            def _rows_for_term(term, col_cleaned):
                matched = df_cleaned[col_cleaned == term]
                return [
                    {'row_num': row['原始行号'], 'source': row[source_col], 'target': row[target_col]}
                    for _, row in matched.iterrows()
                ]

            def _add_similar_entry(term_info, opposite_pairs, term_name, opposite_name,
                                   col_type, similarity, group_id):
                if term_info['row_num'] in similar_rows_seen:
                    return
                if opposite_pairs:
                    rep = opposite_pairs[0]
                    desc = (f"{col_type}相似 {similarity}：'{term_name}' ↔ '{opposite_name}'"
                            f" [对比术语对: {rep['source']} → {rep['target']}]")
                else:
                    desc = f"{col_type}相似 {similarity}：'{term_name}' ↔ '{opposite_name}'"
                category, detail = categorize_issue(desc)
                similar_rows_seen.add(term_info['row_num'])
                uncertain_data.append({
                    '原始行号': term_info['row_num'],
                    source_col: term_info['source'],
                    target_col: term_info['target'],
                    '问题大类': category,
                    '具体问题': detail,
                    '冲突组ID': group_id,
                })
            ...
            for similar_pair in all_similar:
                ...
                group_id   = cluster_to_group_id[find(term1)]
                col_cleaned = col_cleaned_cache[col_name]
                term1_pairs = _rows_for_term(term1, col_cleaned)
                term2_pairs = _rows_for_term(term2, col_cleaned)
                for info in term1_pairs:
                    _add_similar_entry(info, term2_pairs, term1, term2, col_type, similarity, group_id)
                for info in term2_pairs:
                    _add_similar_entry(info, term1_pairs, term2, term1, col_type, similarity, group_id)
        else:
            print("  未发现高度相似的术语")
    ...
    if output_clean is None:
        output_clean = os.path.join(output_dir, f"{base_stem}_cleaned{file_ext}")
    if output_removed is None:
        output_removed = os.path.join(output_dir, f"{base_stem}_removed{file_ext}")
    if output_uncertain is None:
        output_uncertain = os.path.join(output_dir, f"{base_stem}_uncertain{file_ext}")
...
"""
    ...
    # 保存报告（同样写入 Output 目录）
    report_file = os.path.join(output_dir, f"{base_stem}_report.txt")
...
def batch_process_directory(directory=".", config=None):
    ...
    # 保存批量处理报告（写入 Output 子目录）
    batch_output_dir = os.path.join(directory, "Output")
    os.makedirs(batch_output_dir, exist_ok=True)
    batch_report_file = os.path.join(batch_output_dir, f"batch_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt")
