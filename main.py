...
def _ask_config(config, ask_interactive_mode=False):
    """
    交互式询问清洗配置选项，将结果写入 config 对象。

    参数:
        config: 要修改的 CleanConfig 实例
        ask_interactive_mode: 是否额外询问"互为翻译时交互式确认"
                              （单文件模式需要，批量模式不需要）
    """
    print("\n=== 清洗配置 ===")
    config.ignore_case = (
        input("忽略大小写差异？[Y/n，默认Y]: ").strip().lower() not in ('n', 'no')
    )
    config.remove_punctuation = (
        input("去除首尾标点？[Y/n，默认Y]: ").strip().lower() not in ('n', 'no')
    )
    config.quality_check = (
        input("进行质量检查？[Y/n，默认Y]: ").strip().lower() not in ('n', 'no')
    )
    config.similarity_check = (
        input("检测相似术语？[Y/n，默认Y]: ").strip().lower() not in ('n', 'no')
    )

    if ask_interactive_mode:
        config.interactive_mode = (
            input("互为翻译时交互式确认？[y/N，默认N]: ").strip().lower() == 'y'
        )

    if config.similarity_check:
        threshold_input = input(
            f"相似度阈值（0-100，默认{config.similarity_threshold * 100:.0f}）: "
        ).strip()
        if threshold_input:
            try:
...
def main():
    ...
    # 设置日志系统
    try:
        setup_logging(enable_file=True, level='INFO')
    ...
    if len(sys.argv) > 1:
        ...
        if mode == "2":
            ...
            _ask_config(config, ask_interactive_mode=False)
        ...
        _ask_config(config, ask_interactive_mode=True)
