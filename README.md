...
> 所有输出文件统一写入输入文件同级的 `Output/` 子目录，不污染原始文件夹。
...
> 所有交互提示均支持直接按 **Enter** 接受默认值，快速完成配置。

---

## 📁 输出文件说明

> **输出目录**：所有生成文件统一写入输入文件所在目录的 `Output/` 子目录（自动创建）。
> 批量处理时，每个文件的输出写入其自身所在目录的 `Output/` 中，批量汇总报告同样位于 `Output/` 下。
...
### v3.0 (2026-04)
- 📁 **统一输出目录**：所有生成文件自动写入 `Output/` 子目录，不污染原始文件夹
- ⌨️ **默认 Y 交互**：所有命令行交互提示均可直接按 Enter 接受默认值
- 🗂️ **日志文件输出**：运行时自动在 `logs/` 目录生成带时间戳的详细日志
- ♻️ **代码重构**：提取 `_ask_config` 辅助函数，消除重复交互配置代码
- ⚡ **相似术语去重优化**：O(n²) 行扫描改为 O(1) set 查找，大型数据集性能提升明显
- 🧹 **代码清理**：消除 term1/term2 重复处理块，提取嵌套辅助函数
